...
must_haves:
  truths:
    ...
    - "Results displayed as flat list sorted by relevance (NOT grouped by type)"
...
Output: Command palette shows search results as user types, displayed as a FLAT LIST sorted by relevance (per user decision: "Mixed relevance - all results together sorted by relevance/recency, not grouped by type").
</objective>
...
<task type="auto">
  <name>Task 1: Create SearchResults component (flat list, no grouping)</name>
  <files>src/components/command-palette/SearchResults.tsx</files>
  <action>
Create `src/components/command-palette/SearchResults.tsx`:

IMPORTANT: Per user decision in CONTEXT.md, results must be displayed as a FLAT LIST sorted by relevance - NOT grouped by entity type. Do NOT use Command.Group for search results.
...
Key implementation:
- NO Command.Group wrapper - results are flat list sorted by relevance
- Icons for each entity type (user, asset, payee)
- Loading spinner while searching
- Entity type label on the right (to indicate type without grouping)
- Title and subtitle truncate for long text
- Uses cmdk's Command.Item for keyboard navigation
- Results assume pre-sorted by relevance from useGlobalSearch hook
  </action>
  <verify>
1. `npx tsc --noEmit` passes
2. File exports SearchResults
3. No Command.Group in the output - grep confirms flat list structure
  </verify>
  <done>SearchResults component created with flat list (no grouping), entity icons, loading state, and type labels</done>
</task>
...
- Search results shown as FLAT LIST (not grouped) - SearchResults handles this
...
3. Type "john" - should see search results appear as flat list (not grouped by type)
...
- Search results appear while typing as FLAT LIST (mixed relevance, not grouped by type)
...
Test:
1. Press Cmd+K
2. Type "john" - search results should appear as FLAT LIST (no "Users" / "Assets" group headers)
3. Results should show user/asset/payee icons with type label on right
...
<success_criteria>
- Search results appear in command palette as FLAT LIST (not grouped by type)
- Results sorted by relevance (handled by useGlobalSearch hook)
- Results have entity type icons and type label
