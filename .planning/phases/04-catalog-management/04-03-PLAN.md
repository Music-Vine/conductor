...
- Use workflow/transitions to get next state:
  ```typescript
  import { getNextState, isValidTransition } from '@/lib/workflow/transitions'

  // In approve handler:
  const nextState = getNextState(asset.status, 'approve', asset.type)
  if (!nextState) {
    return NextResponse.json({ error: 'Invalid state transition' }, { status: 400 })
  }
  // Update asset.status = nextState
  ```
