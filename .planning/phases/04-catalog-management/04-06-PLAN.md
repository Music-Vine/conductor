...
must_haves:
  ...
  artifacts:
    - path: "src/hooks/useUploader.tsx"
      ...
      exports: ["useUploader", "setFileMetadata", "getFileMetadata"]
...
Includes file metadata storage methods (setFileMetadata/getFileMetadata) to enable validation (Plan 04-05) to store extracted metadata that asset creation (Plan 04-08) can retrieve:
...
```
  ...
  <verify>Hook compiles without TypeScript errors; setFileMetadata and getFileMetadata are exported in return object</verify>
  <done>Uppy hook manages complete upload lifecycle with S3 multipart support and metadata storage for cross-plan data passing</done>
</task>
...
2. useUploader exports setFileMetadata and getFileMetadata methods
3. FileDropzone accepts drops and shows visual feedback
4. FileUploadList displays file progress with all states
5. Retry button appears on error state
6. Remove button only appears on pending/error states
</verification>
