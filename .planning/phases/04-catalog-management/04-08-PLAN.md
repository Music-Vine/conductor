...
NOTE: "Shared metadata" (contributor, genre, tags) is applied to ALL uploaded files per
CONTEXT.md. Title handling is not explicitly specified in CONTEXT, so using Claude's
discretion: titles derive from each file's name (file.name without extension). This is
standard practice for batch uploads where files have meaningful names.
...
Fields (all are shared across files):
...
NOTE: BPM, key, duration, and resolution are PER-FILE metadata extracted via client-side
probing (per user decision). They are NOT part of SharedMetadata.

SharedMetadata interface (only common metadata applied to all files):
```typescript
...
<success_criteria>
- Multi-file upload with shared metadata (contributor, genre, tags applied to all per CONTEXT)
- Per-file metadata (BPM, duration, key, resolution) extracted and stored per-asset (not shared)
- Asset titles derive from filenames (Claude's discretion - standard practice for batch uploads)
- Per-file validation with clear error messages
- Duplicate detection prevents re-uploading same files
- Upload creates asset records with merged shared + per-file metadata
</success_criteria>
