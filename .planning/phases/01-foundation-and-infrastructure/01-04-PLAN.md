...
must_haves:
  truths:
    ...
    - "Platform switches are audit logged when userId is provided"
  artifacts:
...
export function PlatformToggle({ className = '', userId }: PlatformToggleProps) {
  ...
  const handleSwitch = (newPlatform: Platform) => {
    if (newPlatform !== platform) {
      ...
      if (userId) {
        import('@/lib/audit').then(({ captureAuditEvent }) => {
          captureAuditEvent({
            actor: userId,
...
6. PlatformToggle accepts optional userId prop for audit logging (uses dynamic import - audit module from Plan 07)
</verification>
