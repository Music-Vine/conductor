...
/**
 * Validates a magic link token and returns the email if valid.
 *
 * In production, this would also:
 * 1. Check nonce hasn't been used (database lookup)
 * 2. Mark nonce as used
 */
export async function validateMagicLinkToken(token: string): Promise<{ email: string } | null> {
  try {
    ...
    return { email: magicPayload.email }
  } catch {
    return null
  }
}
 ...
 */
export async function requestMagicLink(email: string): Promise<{ success: boolean; debugUrl?: string }> {
 ...
 * Just email field, clear feedback, fast aesthetic.
 */
...
export async function validateMagicLink(token: string): Promise<MagicLinkState> {
  ...
  // Check if Remember Me was set (we'll use a default for now)
  // In production, this would come from the original login form
  const rememberMe = false

  await createSession(mockUser, rememberMe)
